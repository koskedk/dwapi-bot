...
{
    public class ScanSiteHandler : IRequestHandler<ScanSite, Result>
    {
        private readonly IMediator _mediator;
        private readonly IPatientIndexRepository _repository;

        public ScanSiteHandler(IMediator mediator, IPatientIndexRepository repository)
        {
            _mediator = mediator;
            _repository = repository;
        }

        public async Task<Result> Handle(ScanSite request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                while (page <= pageCount)
                {
                    ...
                    await _mediator.Publish(new SiteScanned(request.SiteCode,mpis.Count, totalRecords));
            ...
            {
                Log.Error(e, $"{nameof(ScanSiteHandler)} Error");
